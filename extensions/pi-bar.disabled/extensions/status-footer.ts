...
type SegmentName = "model" | "thinking" | "context" | "progress" | "extensions" | "speed";
type StatusFilter =
...
export default function (pi: ExtensionAPI) {
	...
	let speedOutputText = "";
	let speedReasoningText = "";
	let speedLastOutputCount = 0;
	let speedLastReasoningCount = 0;
	let speedLastUpdateTime = 0;
		...
		return content
			.map((block) => {
				...
				if (r.type === "thinking" || r.type === "reasoning") {
					return typeof r.thinking === "string" ? r.thinking : typeof r.reasoning === "string" ? r.reasoning : "";
				}
				return "";
			})
			.join("");
	...
	pi.on("turn_end", async (event) => {
		if (visibleSegments.includes("speed")) {
			const msg = event.message as Record<string, unknown>;
			const usage = msg?.usage;
			if (usage && typeof usage === "object") {
				const u = usage as Record<string, unknown>;
				const outputTokens = typeof u.outputTokens === "number" ? u.outputTokens : 0;
				const reasoningTokens = typeof u.thinkingTokens === "number" ? u.thinkingTokens : typeof u.reasoningTokens === "number" ? u.reasoningTokens : 0;
				const elapsed = (Date.now() - speedStartTime) / 1000;
				if (elapsed > 0 && outputTokens > 0) {
					speedOutputText = `${Math.round(outputTokens / elapsed)}o/s`;
				}
				if (elapsed > 0 && reasoningTokens > 0) {
					speedReasoningText = `${Math.round(reasoningTokens / elapsed)}r/s`;
				}
			}
		}
		refresh();
	...
	pi.on("message_update", async (event, ctx) => {
		...
		if (visibleSegments.includes("speed") && ctx.hasUI) {
			const content = (event.message as Record<string, unknown>)?.content;
			const outputText = extractAssistantText(content);
			...
			if (speedStartTime === 0) {
				speedStartTime = now;
				speedLastOutputCount = outputTokens;
				speedLastReasoningCount = reasoningTokens;
				speedLastUpdateTime = now;
			} else if (now > speedLastUpdateTime) {
				const elapsed = (now - speedLastUpdateTime) / 1000;
				if (elapsed >= 0.05) {
					const outDiff = outputTokens - speedLastOutputCount;
					if (outDiff > 0) {
						speedOutputText = `${Math.round(outDiff / elapsed)}o/s`;
					}
					const reasoningDiff = reasoningTokens - speedLastReasoningCount;
					if (reasoningDiff > 0) {
						speedReasoningText = `${Math.round(reasoningDiff / elapsed)}r/s`;
					}
					speedLastOutputCount = outputTokens;
					speedLastReasoningCount = reasoningTokens;
					speedLastUpdateTime = now;
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					speed: speedOutputText
					? theme.fg("success", speedOutputText) +
						(speedReasoningText ? " " + theme.fg("muted", speedReasoningText) : "")
					: null,
